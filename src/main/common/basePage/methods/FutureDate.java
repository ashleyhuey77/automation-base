	...
	public String getDate(String format, int... numOfDays) throws Exception {
		...
		try {
			if (numOfDays.length != 0) {
        			...
        			LocalDate dTPlus = LocalDate.now().plusDays(numOfDays[0]);
        			value = dTPlus.format(formatter).toString();
			} else {
				throw new Exception("The total number of days in order to calculate a future date was not provided. Please specify a number of days.");
			}
		} catch (Exception e) {
			throw e;
		}
